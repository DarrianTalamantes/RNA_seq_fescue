...
rule all:
    input:
        ...
        # # Kraken & Star
        expand([
            config["kraken"]["classified"] + "/krakened_{pairs}.fq.gz",
            config["kraken"]["fungal"] + "/{pairs}R1.fq",
            config["kraken"]["fungal"] + "/{pairs}R2.fq",
            config["kraken"]["non_fungal"] + "/{pairs}R1.fq",
            config["kraken"]["non_fungal"] + "/{pairs}R2.fq"
        ], pairs=PAIRS) +
        [
        ...
        expand([
            config["directories"]["sep_bams"] + "{pairs}Aligned.sortedByCoord.out.bam",
            config["directories"]["sep_bams"] + "{pairs}Log.out",
            config["directories"]["sep_bams"] + "{pairs}Log.final.out",
            config["directories"]["sep_bams"] + "{pairs}SJ.out.tab"
        ], pairs=PAIRS) +
